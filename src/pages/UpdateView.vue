...
<script>
  ...
  export default {
    setup() {
      ...
      const getInfo = () => {
        fetch(`http://sm6683.dothome.co.kr/data_read_id.php?id=${route.params.id}`)
          .then(res => res.json())
          .then(data => {
            // console.log(data);
      ...
      const onSubmit = () => {
        ...
        fetch(
            `http://sm6683.dothome.co.kr/data_update.php?id=${todo.id}&title=${todo.title}&body=${todo.body}&complete=${todo.complete}`
            )
          .then(res => res.json())
          .then(data => {
