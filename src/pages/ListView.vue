    ...
    export default {
        ...
        setup() {
            ...
            const getInfo = (_page = 1) => {
                ...
                fetch(
                        `http://sm6683.dothome.co.kr/data_read.php?page_now=${page_now.value}&data_count=${data_count}`)
                    .then(res => res.json())
                    .then(data => {
                        // 배열데이터를 보관한다.    
            ...
            const deleteTodo = () => {
                // console.log(_id);
                fetch(`http://sm6683.dothome.co.kr/data_delete.php?id=${deleteId.value}`)
                    .then(res => res.json())
                    .then(data => {
                        // console.log(data);
                        // 목록갱신
            ...
            const toggleTodo = (_obj) => {
                ...
                fetch(
                        `http://sm6683.dothome.co.kr/data_update.php?id=${_obj.id}&title=${_obj.title}&body=${_obj.body}&complete=${_obj.complete}`)
                    .then(res => res.json())
                    .then(data => {
                        console.log(data)
                    })
            ...
            // 전체 데이터 수 받아오기
            const getTotal = () => {
                fetch(`http://sm6683.dothome.co.kr/data_total.php`)
                    .then(res => res.json())
                    .then(data => {
                        // 전체 데이터 수 갱신
            ...
            const getTotalSearch = () => {
                ...
                clearTimeout(searchTimer);
                fetch(`http://sm6683.dothome.co.kr/data_total_search.php?title=${searchTxt.value}`)
                    .then(res => res.json())
                    .then(data => {
                        // 검색어에 해당하는 데이터 개수
                        data_total.value = data.total;
            ...
            const searchTodo = () => {
                fetch(
                        `http://sm6683.dothome.co.kr/data_read_search.php?page_now=${page_now.value}&data_count=${data_count}&title=${searchTxt.value}`)
                    .then(res => res.json())
                    .then(data => {
